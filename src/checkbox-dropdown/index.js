...
const [createComponent, bem] = createNamespace('checkbox-dropdown');
...
const CheckboxDropdown = createComponent({
      ...
      const selectedLabels = this.internalValue
        .map((val) => {
          const selected = options.find(
            (item) => item[this.currentValueKey] === val
          );
  ...
  methods: {
    ...
    onFieldClick() {
      ...
      this.showPopup = true;
      // 打开时复制当前值到临时值
      this.tempValue = [...this.internalValue];
    ...
    genSelectedTags() {
      ...
      const options = this.getCurrentOptions();
      const selectedItems = this.tempValue.map((val) => {
        const item = options.find((opt) => opt[this.currentValueKey] === val);
        return {
          value: val,
          label: item ? item[this.currentLabelKey] : val,
        };
      });

      return (
        <div class={bem('selected-tags')}>
          <div class={bem('selected-tags-scroll')}>
            {selectedItems.map((item) => (
              <Tag
                ...
                size="medium"
                closeable
                class={bem('selected-tag')}
                onClose={() => this.onRemoveTag(item.value)}
              >
                {item.label}
              </Tag>
            ))}
      ...
      const listContent = (
        ...
        >
          {this.filteredOptions.map((item) => (
            <Cell key={item[this.currentValueKey]} clickable>
              <Checkbox name={item[this.currentValueKey]}>
    ...
    genListActions() {
      return (
        <div class={bem('list-actions')}>
          <Button
            type="default"
            size="small"
            class={bem('list-btn')}
            onClick={this.onListReset}
          >
            重置
          </Button>
          <Button
          ...
          <div class={bem('list-popup')}>
            <div class={bem('list-content')}>
              <CheckboxGroup
                ...
                onInput={this.onCheckboxChange}
              >
                {this.genListContent()}
              </CheckboxGroup>
            </div>
            {this.genSelectedTags()}
            {this.genListActions()}
          </div>
        </Popup>
      );
